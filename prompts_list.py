...
def system(name, role, char, mafianum, doctornum, playernum, mafia_players):
    ...
    prompt += f"\nThe person/character you are emulating is {char}. Speak as if you were {char}, and try to act in character as much as possible. You should not talk about who you are emulating or who anyone else is emulating. Focus on the game of mafia."
    return prompt

def vote(players):
    return f"Based on the current situation, who would you like to vote for? Here are the players currently still alive (including yourself): {', '.join([p.name for p in players])}. You can also choose to abstain from voting, or vote to skip eliminating anyone. To vote, state the name of the player you want to vote for, followed by 'LLAMALLAMA' (e.g., 'Llama # LLAMALLAMA') (match caps). To abstain from voting, type 'abstain LLAMALLAMA' (match caps). To skip eliminating anyone, type 'skip LLAMALLAMA' (match caps). Do not justify your vote. Do not type anything more than your vote."
